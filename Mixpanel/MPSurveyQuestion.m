...
{
    if (object == nil) {
        CoreLogType(WBLogLevelFatal, WBLogTypeMixpanelAnalytics, @"question json object should not be nil");
        return nil;
    }
    NSNumber *ID = object[@"id"];
    if (!([ID isKindOfClass:[NSNumber class]] && [ID integerValue] > 0)) {
        CoreLogType(WBLogLevelFatal, WBLogTypeMixpanelAnalytics, @"invalid question id: %@", ID);
        return nil;
    }
    NSString *prompt = object[@"prompt"];
    if (![prompt isKindOfClass:[NSString class]]) {
        CoreLogType(WBLogLevelFatal, WBLogTypeMixpanelAnalytics, @"invalid question prompt: %@", prompt);
        return nil;
    }
    NSDictionary *extraData = object[@"extra_data"];
    if (![extraData isKindOfClass:[NSDictionary class]]) {
        CoreLogType(WBLogLevelFatal, WBLogTypeMixpanelAnalytics, @"invalid question extra data: %@", extraData);
        return nil;
    }
    NSString *type = object[@"type"];
    if (![type isKindOfClass:[NSString class]]) {
        CoreLogType(WBLogLevelFatal, WBLogTypeMixpanelAnalytics, @"invalid question type: %@", type);
        return nil;
    }
    ...
    CoreLogType(WBLogLevelFatal, WBLogTypeMixpanelAnalytics, @"unknown question type: %@", type);
    return nil;
}
...
- (id)initWithID:(NSUInteger)ID type:(NSString *)type prompt:(NSString *)prompt andChoices:(NSArray *)choices
{
    if (choices != nil && [choices count] > 0) {
        ...
        CoreLogType(WBLogLevelFatal, WBLogTypeMixpanelAnalytics, @"invalid choices: %@", choices);
    }
    return self;
}
